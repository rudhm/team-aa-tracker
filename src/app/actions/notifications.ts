...
export async function processTaskNotification(taskId: string) {
  ...
  if (task.notified_editor !== task.editor) {
    try {
      ...
      // Reset urgent notification state for the new editor
  ...
  // If it was just assigned and is urgent, we also send the urgent ping to make sure they see it.
  ...
  if (isNewlyUrgent && (task.notified_editor === task.editor || emailSent)) {
    try {
      ...
      console.error("Failed to send urgent email", err)
    }
  }
